...
function persistWhitelist(id, isWhitelisted): void {
...
function checkWhitelist(id: string): Promise<void> {
    logger.info(`Checking whitelist for server with id ${id}`);
    const connectionInfo: Array<string> = id.split(':');
    const ipAddress: string = connectionInfo[0];
    const port: number = Number(connectionInfo[1]);
    if (port != 25565) {
        return;
    }
 ...
 */
function isIdUnique(id): Promise<boolean> {
...
async function parseResult(result: any): Promise<void> {
    let id: string = `${result.ip_str}:${result.port}`;
...
function gatherServers(query: string, apiKey: string) {
